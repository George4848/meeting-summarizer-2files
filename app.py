...
# ---- Make ffmpeg available without packages.txt (Streamlit Cloud friendly)
...
def setup_nlp():
    ...
    # NLTK sentence tokenizer resources (newer NLTK may require punkt_tab)
    ...
    # spaCy: prefer en_core_web_sm if available; otherwise use lightweight blank pipeline
    try:
...
def extract_action_items(summary: str):
    sents = segment_sentences(summary) if summary.strip() else []
...
with st.sidebar:
    ...
    improve_readability = st.toggle("Improve readability (sentence cleanup)", value=True)
...
with tab1:
    ...
    if media_file:
        ...
        if st.button("1) Transcribe with Whisper"):
            suffix = os.path.splitext(media_file.name)[1] or ".wav"
